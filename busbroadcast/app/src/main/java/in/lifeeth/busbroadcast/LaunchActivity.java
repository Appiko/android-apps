...
public class LaunchActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(store instanceof Store){
            Log.d("BBROAD:", "Initialized DB");
            Log.d("BBROAD:", "Last Received SMS "+store.FilesToPlay);
        }else{
            Log.d("BBROAD:","Empty DB - Initializing");
            store = new Store();
            store.FilesToPlay="";
            store.Interval=0;
            store.Count=0;
            ...
            public void onReceive(Context c, Intent i) {
                ...
                for (String File : store.FilesToPlay.split(",")){ // These are pre-verified to be clean from SmsReceiver.java
                    try {
                       ...
                       Log.d("BBROAD:","Exception loading audio files");
                    }
                }
                ...
                if (!sleep) {
                    if (toPlay.length > 0) { // Play only if we have a queued audio list.
                        ...
                        try {
                            ...
                            Log.d("BBROAD:", "You might not set the URI correctly!");
                            ...
                            public void onCompletion(MediaPlayer mp) {
                                ...
                                if (counter < toPlay.length) {
                                    try {
                                        ...
                                        Log.d("BBROAD:", "You might not set the URI1 correctly!");
                                    } catch (SecurityException e) {
                                        Log.d("BBROAD:", "You might not set the URI1 correctly!");
                                    } catch (IllegalStateException e) {
                                        Log.d("BBROAD:", "You might not set the URI1 correctly!");
                    ...
                    if(store.CompletedPlays==1){ // Send SMS back on first play.
                        ...
                        Log.d("BBROAD:", "Sending back confirmation!");
                    }
                }
            }
        };

        registerReceiver(cronReceiver, new IntentFilter("in.lifeeth.cronservice"));
        cronPendingIntent = PendingIntent.getBroadcast(this, 0, new Intent("in.lifeeth.cronservice"), 0);
